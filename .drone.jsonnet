local Build(target, py_version, target_branch='develop', tests_dir='tests/', excludes=[]) = {
  ...
  kind: 'pipeline',
  name: std.format('%s py%s %s', [target, py_version, std.strReplace(tests_dir, 'tests/', '')]),
  steps: [
    {
      name: 'tox',
      image: std.format('saltstack/au-%s:ci-%s', [target, target_branch]),
      commands: [
        std.format(
          'tox -e py%s-pytest -- --ssh-tests --run-destructive -vv -ra -s --sysinfo --sys-stats --log-cli-level=info %s',
          [py_version, tests_dir]
        ),
      ],
    },
  ],
